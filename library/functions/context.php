...
function hybrid_body_class( $class = '' ) {
	...
	if ( is_singular() ) {
		...
		$template = sanitize_html_class( str_replace( array ( "{$post->post_type}-template-", "{$post->post_type}-", '.php' ), '', get_post_meta( get_queried_object_id(), "_wp_{$post->post_type}_template", true ) ) );
		if ( !empty( $template ) )
